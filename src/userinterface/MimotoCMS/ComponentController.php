...
//
//        atal error: Call to a member function getPropertySelector() on null in /Users/sebastiankersten/Development/Webroot/Mimoto.Aimless/src/domains/Aimless/AimlessComponent.php on line 1123

        // 3. prepare
        //$eComponent = Mimoto::nEntityId'onCreatedConnectTo' => CoreConfig::MIMOTO_ENTITY.'.'.$nEntityId.'.components',
        ...
        // 4. create
        $eComponent = Mimoto::service('data')->create(CoreConfig::MIMOTO_COMPONENT);
        Mimoto::service('data')->store($eComponent);

        // 5. connect to root
        $eRoot = Mimoto::service('data')->get(CoreConfig::MIMOTO_ROOT, CoreConfig::MIMOTO_ROOT);
        $eRoot->addValue('components', $eComponent);
        ...
        if (MimotoDataUtils::isValidId($nEntityId))
        {
            ...
            if (!empty($eEntity))
            {
                $eEntity->addValue('components', $eComponent);
